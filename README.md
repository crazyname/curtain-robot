...
- 微信小程序：作为移动端交互入口，当前已有基础工程和蓝牙页面代码，完整控制闭环仍需继续打通。
- Python 边缘控制：承担串口通信、自动控制、定时控制、传感器读取和本地策略组织。
- STM32 固件：承担电机、传感器、RTC、串口收发和底层执行控制。
- 电机与机械结构：对应窗帘运动执行机构、外壳结构、PCB/EDA 资料和 3D 打印结构文件。

串口命令和跨端通信约定见 [串口协议草案](docs/串口协议草案.md)。
